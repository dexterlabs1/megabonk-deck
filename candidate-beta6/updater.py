#!/usr/bin/env python3
"""Offline beta 6 candidate updater. Bundled hashes are inserted by build.py."""
import fcntl
import hashlib
import json
import os
...
PACKAGE_NAME = 'megabonk-deck-beta6.zip'
...
def apply_update(game, state, package):
    ...
    if current_sha in PREVIOUS_BETA_SHAS | {BETA_DLL_SHA}:
        ...
        raise RuntimeError('Unknown mod version. Expected official 5.1.0 or Deck beta 1, 2, 3, 4, or 5. No files changed.')
    with tempfile.TemporaryDirectory(prefix='megabonk-deck-candidate-') as temp:
        ...
        if running('Megabonk.exe') or target.read_bytes() != current:
            raise RuntimeError('Close Megabonk and any other updater, then try again.')
        backup = state / 'backups' / ('deck-beta6-' + str(time.time_ns()))
