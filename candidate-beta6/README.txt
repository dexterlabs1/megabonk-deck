Megabonk Deck beta 6 - test candidate

Fixes the generated-array signature mismatch found during live controller testing,
restricts focus to the open modal, and adds a visible managed selection marker.
Releases the hidden multiplayer popup before entering character selection so it
cannot continue intercepting character-menu input while its cleanup is pending.
Keeps the earlier hover, navigation, and separate room-code layout changes.
Layout, direct text entry, and solo hosting have been verified on a physical Deck.
A fresh installation, the built-in STEAM+X keyboard, and two-player gameplay
remain unverified on the Deck. The final selection marker needs live verification.

EXISTING Together installation (official Proton 5.1.0 or Deck beta 1-5):
1. Switch to Desktop Mode and close Megabonk. Steam can stay open.
2. Extract the WHOLE megabonk-deck-beta6-test.zip archive in Downloads.
3. Open the extracted folder and double-click Test-Deck-Beta6.desktop.
...
4. Confirm the update. Launch Megabonk in Gaming Mode and check Deck beta 6.
...
4. After the original installer succeeds, double-click Test-Deck-Beta6.desktop
   in the same folder. Confirm the update, then launch the game in Gaming Mode.
5. Check that the menu says Deck beta 6.
...
restore launchers do not recognize the beta 6 DLL.
...
megabonk-deck-beta6-source.zip contains the matching modified mod source and
