...
Beta 7 is published as prerelease `deck-beta7` from `4beb033`. All six public release assets were downloaded without authentication and hash-verified. The user's Deck is on the final managed beta 7 installation with official restore verified. Packaging checks passed: 18 candidate, 15 friend-installer and 21 remote integration tests. When creating a release with `gh --target`, pass the full commit SHA; the API rejected the abbreviated SHA.
