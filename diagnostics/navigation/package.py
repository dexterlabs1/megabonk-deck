"""Package the built beta 4 candidate, preserving beta 3 source membership."""
import difflib
import hashlib
import json
...
changed = (
    ...
    "src/plugin/Scripts/Button/PlayTogetherButton.cs",
)
with ZipFile(repo / "mod-source/megabonk-deck-beta3-source.zip") as baseline:
...
patch = ["Deck beta 3 -> Deck beta 4 candidate (incremental, not upstream-to-beta).\n",
...
for name in changed:
    ...
    files[name] = data
(repo / "mod-source/deck-beta4.patch").write_text("".join(patch), encoding="utf-8", newline="\n")
...
source_zip = repo / "mod-source/megabonk-deck-beta4-source.zip"
binary_zip = repo / "releases/megabonk-deck-beta4.zip"
...
hashes = {str(p.relative_to(repo)).replace("\\", "/"): hashlib.sha256(p.read_bytes()).hexdigest()
          for p in [binary_zip, source_zip, repo / "mod-source/deck-beta4.patch"]}
hashes["MegabonkTogether.dll"] = hashlib.sha256(dll).hexdigest()
(repo / "diagnostics/navigation/candidate-sha256.json").write_text(json.dumps(hashes, indent=2) + "\n")
