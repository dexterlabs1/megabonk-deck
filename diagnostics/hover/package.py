"""Package the built beta 3 candidate, preserving beta 2 source membership."""
import difflib
import hashlib
import json
...
changed = (
    ...
    "src/plugin/Scripts/Button/CustomButton.cs",
)
with ZipFile(repo / "mod-source/megabonk-deck-beta2-source.zip") as baseline:
...
patch = ["Deck beta 2 -> Deck beta 3 candidate (incremental, not upstream-to-beta).\n",
         "Apply with git apply -p1 from the extracted beta 2 source root.\n\n"]
for name in changed:
    ...
    files[name] = data
(repo / "mod-source/deck-beta3.patch").write_text("".join(patch), encoding="utf-8", newline="\n")
...
source_zip = repo / "mod-source/megabonk-deck-beta3-source.zip"
binary_zip = repo / "releases/megabonk-deck-beta3.zip"
...
hashes = {str(p.relative_to(repo)).replace("\\", "/"): hashlib.sha256(p.read_bytes()).hexdigest()
          for p in [binary_zip, source_zip, repo / "mod-source/deck-beta3.patch"]}
hashes["MegabonkTogether.dll"] = hashlib.sha256(dll).hexdigest()
(repo / "diagnostics/hover/candidate-sha256.json").write_text(json.dumps(hashes, indent=2) + "\n")
