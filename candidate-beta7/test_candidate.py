"""Linux integration tests with real DLLs. Run after candidate-beta7/build.py.

MEGABONK_OFFICIAL_ZIP=/path/to/Proton-5.1.0.zip python3 -m unittest discover -s candidate-beta7 -v
...
class CandidateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.bundle = zipfile.ZipFile(ROOT / 'releases/megabonk-deck-beta7-test.zip')
        cls.prefix = 'megabonk-deck-beta7-test/'
        cls.u = types.ModuleType('candidate_updater')
        exec(cls.bundle.read(cls.prefix + 'candidate_updater.py'), cls.u.__dict__)
        cls.package = cls.bundle.read(cls.prefix + 'megabonk-deck-beta7.zip')
    ...
    def test_beta6_upgrade_restore(self):
        self.check_upgrade(6)
    ...
    def test_beta1_beta2_beta3_beta4_beta5_beta6_beta7_restore(self):
    ...
    def test_bundle_manifest_and_source_license(self):
        ...
        self.assertEqual(self.bundle.read(self.prefix + 'megabonk-deck-beta7-source.zip'), (ROOT / 'mod-source/megabonk-deck-beta7-source.zip').read_bytes())
        self.assertEqual(self.package, (ROOT / 'releases/megabonk-deck-beta7.zip').read_bytes())
        self.assertEqual(self.u.PREVIOUS_BETA_SHAS, {sha(dll) for _, dll in self.betas.values()})
        with zipfile.ZipFile(io.BytesIO(self.bundle.read(self.prefix + 'megabonk-deck-beta7-source.zip'))) as source:
    ...
    def test_actual_launcher_install_restore_from_other_cwd(self):
        ...
        folder = self.home / 'Downloads/megabonk-deck-beta7-test'
        for uri in (False, True):
            with self.subTest(file_uri=uri):
                original = self.old_beta(4)
                for filename in ('Test-Deck-Beta7.desktop', 'Restore-Official-Multiplayer.desktop'):
