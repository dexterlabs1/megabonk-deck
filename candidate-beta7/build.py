...
def launcher(updater, restore=False):
    ...
    name = 'Restore Official Megabonk Multiplayer' if restore else 'Test Megabonk Deck Beta 7'
...
def build():
    package = (ROOT / 'releases/megabonk-deck-beta7.zip').read_bytes()
    source = (ROOT / 'mod-source/megabonk-deck-beta7-source.zip').read_bytes()
    with zipfile.ZipFile(io.BytesIO(package)) as archive:
        dll = archive.read('MegabonkTogether.dll')
    updater = (ROOT / 'candidate-beta7/updater.py').read_text(encoding='utf-8')
    ...
    files = {
        'Test-Deck-Beta7.desktop': (launcher(updater), 0o755),
        'Restore-Official-Multiplayer.desktop': (launcher(updater, restore=True), 0o755),
        'megabonk-deck-beta7.zip': (package, 0o644),
        'megabonk-deck-beta7-source.zip': (source, 0o644),
        ...
        'README.txt': ((ROOT / 'candidate-beta7/README.txt').read_bytes(), 0o644),
    }
    ...
    out = ROOT / 'releases/megabonk-deck-beta7-test.zip'
    with zipfile.ZipFile(out, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for name, (data, mode) in sorted(files.items()):
            info = zipfile.ZipInfo('megabonk-deck-beta7-test/' + name, (2026, 9, 21, 0, 0, 0))
