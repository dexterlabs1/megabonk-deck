#!/usr/bin/env python3
"""Offline beta 7 candidate updater. Bundled hashes are inserted by build.py."""
import fcntl
import hashlib
import json
import os
...
PREVIOUS_BETA_SHAS = {
    'c79cf7d1abf4b25c02ee8b51d7d754f57a98a65fb213fad0464059d446af4580',
    '27aad034b86cdde08c1d58689fabbf686de7a003dfcad9bc431e69dbdedb93a7',
    ...
    '9405969530e6ea76f65d6ffb3a0ee79bfa3a39587b17fe3063e3d15f303e1c71',
...
PACKAGE_NAME = 'megabonk-deck-beta7.zip'
...
def apply_update(game, state, package):
    ...
    if current_sha in PREVIOUS_BETA_SHAS | {BETA_DLL_SHA}:
        ...
        raise RuntimeError('Unknown mod version. Expected official 5.1.0 or Deck beta 1, 2, 3, 4, 5, or 6. No files changed.')
    with tempfile.TemporaryDirectory(prefix='megabonk-deck-candidate-') as temp:
        ...
        if running('Megabonk.exe') or target.read_bytes() != current:
            raise RuntimeError('Close Megabonk and any other updater, then try again.')
        backup = state / 'backups' / ('deck-beta7-' + str(time.time_ns()))
...
def main():
    ...
    with (state / 'install.lock').open('w') as lock:
        ...
        if not dialog('Install Deck beta 7 test candidate?\n\nFixes built-in controller navigation immediately after launching the game by leaving native main-menu cursor ownership to the game. Keeps the beta 6 modal and room-code fixes. Beta 7 still needs testing on a physical Deck.\n\nOnly the multiplayer DLL changes; your official 5.1.0 backup is preserved. Use the Restore-Official-Multiplayer launcher in this bundle to undo it. Steam can stay open.', True):
            return
        changed = apply_update(game, state, package)
        dialog(('Updated!' if changed else 'This candidate is already installed.') + '\n\nLaunch Megabonk in Gaming Mode. The menu should say Deck beta 7.\n\nOpen Together! > Friendlies and check that the room-code field, Paste Code, Join, and Back are separate. Select the room-code field, use STEAM+X to type a code, then Join. With your friend, test hosting and character selection. Host Confirm requires at least two players in the lobby. Use this bundle\'s Restore-Official-Multiplayer launcher if needed.')
