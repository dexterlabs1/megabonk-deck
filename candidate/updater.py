#!/usr/bin/env python3
"""Offline beta 3 candidate updater. Bundled hashes are inserted by build.py."""
import fcntl
import hashlib
import json
import os
...
PACKAGE_NAME = 'megabonk-deck-beta3.zip'
...
def apply_update(game, state, package):
    ...
    if current_sha in PREVIOUS_BETA_SHAS | {BETA_DLL_SHA}:
        ...
        raise RuntimeError('Unknown mod version. Expected official 5.1.0, Deck beta 1, or Deck beta 2. No files changed.')
    with tempfile.TemporaryDirectory(prefix='megabonk-deck-candidate-') as temp:
        ...
        if running('Megabonk.exe') or target.read_bytes() != current:
            raise RuntimeError('Close Megabonk and any other updater, then try again.')
        backup = state / 'backups' / ('deck-beta3-' + str(time.time_ns()))
...
def main():
    ...
    with (state / 'install.lock').open('w') as lock:
        ...
        if not dialog('Install Deck beta 3 test candidate?\n\nRemoves recursive button hover calls suspected in the character-selection freeze. This is not yet verified on a physical Deck.\n\nOnly the multiplayer DLL changes; your official 5.1.0 backup is preserved. Use the Restore-Official-Multiplayer launcher in this bundle to undo it. Steam can stay open.', True):
            return
        changed = apply_update(game, state, package)
        dialog(('Updated!' if changed else 'This candidate is already installed.') + '\n\nLaunch Megabonk in Gaming Mode. The menu should say Deck beta 3.\n\nHost a room and test character selection. Use this bundle\'s Restore-Official-Multiplayer launcher if needed.')
