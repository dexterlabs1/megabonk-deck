...
def launcher(updater, restore=False):
    ...
    name = 'Restore Official Megabonk Multiplayer' if restore else 'Test Megabonk Deck Beta 3'
...
def build():
    package = (ROOT / 'releases/megabonk-deck-beta3.zip').read_bytes()
    source = (ROOT / 'mod-source/megabonk-deck-beta3-source.zip').read_bytes()
    with zipfile.ZipFile(io.BytesIO(package)) as archive:
        dll = archive.read('MegabonkTogether.dll')
    updater = (ROOT / 'candidate/updater.py').read_text(encoding='utf-8')
    ...
    files = {
        'Test-Deck-Beta3.desktop': (launcher(updater), 0o755),
        'Restore-Official-Multiplayer.desktop': (launcher(updater, restore=True), 0o755),
        'megabonk-deck-beta3.zip': (package, 0o644),
        'megabonk-deck-beta3-source.zip': (source, 0o644),
        'candidate_updater.py': (updater.encode(), 0o644),
        'installer.py': ((ROOT / 'installer.py').read_bytes(), 0o644),
        'README.txt': ((ROOT / 'candidate/README.txt').read_bytes(), 0o644),
    }
    ...
    out = ROOT / 'releases/megabonk-deck-beta3-test.zip'
    with zipfile.ZipFile(out, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for name, (data, mode) in sorted(files.items()):
            info = zipfile.ZipInfo('megabonk-deck-beta3-test/' + name, (2026, 9, 21, 0, 0, 0))
