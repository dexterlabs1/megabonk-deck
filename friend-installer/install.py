#!/usr/bin/env python3
"""One confirmation for the audited official installer followed by Deck beta 5."""
import fcntl
import hashlib
import json
import os
...
PACKAGE_URL = 'https://github.com/dexterlabs1/megabonk-deck/releases/download/deck-beta5/megabonk-deck-beta5.zip'
PACKAGE_SHA = 'd2fc3f9c939a723e07b6daf314f46d56b17e433c4c3b9fa902612f7c763db6e3'
...
def get_package():
    ...
    if hashlib.sha256(package).hexdigest() != PACKAGE_SHA:
        raise RuntimeError('Beta 5 download checksum mismatch. No game files changed.')
    if updater.digest(package) != updater.PACKAGE_SHA:
        raise RuntimeError('Beta 5 package checksum mismatch. No game files changed.')
    return package
...
def main():
    ...
    message = ('Install Megabonk Together with Deck beta 5?\n\n'
               ...
               + 'Beta 5 is a test version. Deck typing and two-player play still need testing.\n\nGame: ' + str(game))
    ...
    with (state / 'install.lock').open('w') as lock:
        ...
        if installer.running('Megabonk.exe'):
            raise RuntimeError('Close Megabonk and run this again to finish beta 5.')
        installed_kind(game, state)
        changed = updater.apply_update(game, state, package)
    installer.dialog(('Installed Deck beta 5!' if changed else 'Deck beta 5 is already installed.')
                     + '\n\nLaunch Megabonk normally in Gaming Mode. The first modded launch can take several minutes. The menu should say Deck beta 5.\n\n'
...
if __name__ == '__main__':
    try:
        ...
        installer.dialog('Installation stopped: ' + str(exc) + '\n\nBeta 5 completion was not confirmed. If Steam closed, reopen it normally. Your backups are retained.')
        sys.exit(1)
